...
class LiveOneFrameSpec extends AnyFreeSpec with EitherValues with MockFactory with Matchers with PrivateMethodTester {
  ...
  val jsonResonse = """[{"from":"EUR","to":"CHF","bid":0.6262930151477809,"ask":0.041126043135137236,"price":0.333709529141459068,"time_stamp":"2020-06-22T07:54:49.451Z"}]"""
  val multipleResultsJson = """[{"from":"CHF","to":"EUR","bid":0.27822164078398093,"ask":0.019623759432511045,"price":0.1489227001082459875,"time_stamp":"2020-06-22T13:04:42.912Z"},{"from":"USD","to":"EUR","bid":0.9460985783654299,"ask":0.33446008550191075,"price":0.640279331933670325,"time_stamp":"2020-06-22T13:04:42.912Z"}]"""
  ...
  implicit val backend: SttpBackend[IO, Nothing, NothingT] = AsyncHttpClientCatsBackend.stub[IO].whenRequestMatchesPartial {
    ...
    case r if r.uri.toString().endsWith("rates?pair=CHFEUR&pair=USDEUR") =>
      successResponse.copy(body = multipleResultsJson)
  ...
  "OneFrame " - {
    "should correctly decode json response from OneFrame" in {
      ...
      (oneFrame invokePrivate decodeContent(jsonResonse)) should be(Right(NonEmptyList(rate, List.empty)))
    }
    ...
    "should correctly return multiple results if data is available" in {
      ...
      val result: Either[errors.Error, NonEmptyList[Rate]] = oneFrame.get(NonEmptyList(Rate.Pair(CHF, EUR), List(Rate.Pair(USD, EUR)))).unsafeRunSync()
